/**
 * Codec functions for Non-IP Modems based on Compact Binary Codec
 * https://github.com/inmarsat-enterprise/compact-binary-codec
 * @namespace cbc
 */
      ...
      if (Object.keys(cFieldKeys).includes(v)) {
        ...
        for (const fKey of mKeys) {
          if (!Object.hasOwn(candidate, fKey)) {
            console.error(`Missing type-conditional key ${fKey}`);
            return false;
          }
          ...
          if (fKey === 'size') {
            ...
            if (!isObject_(fv)) {
              console.error(`Invalid enum definition`);
              return false;
            }
            if (!candidate.size) {
              console.error(`Enum missing size`);
              return false;
            }
            ...
            if (Object.keys(fv).length > maxEntries) {
              console.error(`Enum entries larger than max size`);
              return false;
            }
            for (const [ek, ev] of Object.entries(fv)) {
              if (!isInt(ek, true)) {
                console.error(`Enum key must be parsable as integer`);
                return false;
              }
              if (parseInt(ek) < 0 || parseInt(ek) >= maxEntries) {
                console.error(`Enum key out of range`);
                return false;
              }
              if (!isValidString_(ev)) {
                console.error(`Invalid enum value must be non-empty string`);
                return false;
              }
            }
            ...
            if (duplicates.length > 0) {
              console.error(`Duplicate enum value`);
              return false;
            }
          } else if (fKey === 'fields') {
            if (!Array.isArray(fv)) {
              console.error(`Invalid fields must be non-empty array`);
              return false;
            }
...
function isValidMessageDef_(candidate) {
  ...
  // const coapMessageKeys = ['coapVersion', 'coapType', 'coapTokenLength',
  ...
  for (const key of mMessageKeys) {
    ...
    if (key === 'name') {
      if (!isValidString_(v)) {
        console.error('Invalid message name');
        return false;
      }
    } else if (key === 'direction') {
      if (![MessageDirection.MO, MessageDirection.MT].includes(v)) {
        console.error('Invalid message direction');
        return false;
      }
    } else if (key === 'messageKey') {
      if (typeof v !== 'number' || v < 49152 || v > 65535) {
        console.error(`Invalid or out-of-range messageKey value`);
        return false;
      }
      if (v > 65279) {
        const viasatKey = process.env.VIASAT_KEY;
        if (!Object.hasOwn(candidate, 'viasatKey') ||
            candidate.viasatKey !== viasatKey) {
          console.error(`Reserved messageKey`);
          return false;
        }
      }
    } else if (key === 'fields') {
  ...
  for (const key of oMessageKeys) {
    if (Object.hasOwn(candidate, key)) {
      ...
      if (key === 'description') {
        if (!isValidString_(v)) {
          console.error(`Invalid description must be non-empty string if present`);
          return false;
        }
      }
    }
  ...
  //   for (const key of coapMessageKeys) {
  //     if (!Object.hasOwn(candidate, key))
  //       return false;
  //     const v = candidate[key];
  //     if (key === 'coapVersion') {
