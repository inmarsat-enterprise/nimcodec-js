...
function defaultFieldValue(field) {
  ...
  if (field.type === 'array')
    return [];
  throw new Error(`No default value for type ${field.type}`);
}
...
function decodeField(field, buffer, offset) {
  ...
  if (Object.hasOwn(field, 'optional')) {
    fieldPresent = extractBits(buffer, offset, 1) == 1;
    offset += 1;
  }
  let decoded;
  let value;
...
function encodeField(field, value, buffer, offset) {
  ...
  if (Object.hasOwn(field, 'optional')) {
    if (field.optional != false) {
      present = typeof value != 'undefined' ? true : false;
      ({buffer, offset} = appendBits([Number(present)], buffer, offset));
    }
  }
  if (present)
