/**
 * Functions related to Compact Binary Codec message encoding/decoding
 */
 ...
 * @param {CbcCodec|MessageCodec} codec The codec with messages, or Message codec
 ...
 * @private
 * @param {MessageDecoded} decoded The message object to search
 * @param {Object} encoded.value The message field key/value pairs
 * @param {Field} fieldDef The field definition to search for
 * @returns {any}
 */
function getFieldValue_(decoded, fieldDef) {
  for (const [k, v] of Object.entries(decoded.value)) {
 ...
 * @param {CbcCodec|MessageCodec} codec The codec with messages, or 
 *    Message codec
...
function encodeMessage(message, codec, isMo = false, isCoap = false) {
  ...
  for (const fieldCodec of msgCodec.fields) {
    const value = getFieldValue_(message, fieldCodec);
